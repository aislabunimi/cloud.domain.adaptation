...
for i in range(8, 10):
    ...
    images = sorted(os.listdir(os.path.join(path, 'pseudo5')), key=lambda x: int(os.path.basename(x)[:-4]))
    print(len(images[int(len(images)*0.8):]))
    for image in tqdm(images[:int(len(images)*0.8)]):
        label_sam = cv2.imread(os.path.join(path, 'samCb5', image), cv2.IMREAD_UNCHANGED)-1
